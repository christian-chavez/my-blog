...
function getIntegrationsFromOptions(options) {
  const isTracingEnabled = core.hasTracingEnabled(options);
...
function getIntegrationsFromArray(userIntegrations, isTracingEnabled) {
  if (isTracingEnabled && !userIntegrations.some(integration => integration.name === react.BrowserTracing.name)) {
    userIntegrations.push(new react.BrowserTracing());
  }
...
exports.getIntegrationsFromOptions = getIntegrationsFromOptions;
